...
import { tick, register, loadState, saveState } from './src/agent.js';
...
async function cmdStatus() {
  ...
  out(`today (${state.day}): ${state.postsToday}/${2} posts, ${state.commentsToday}/${6} comments, ${state.dmsToday}/${2} dms`);
  try {
...
const commands = {
  ...
  on: () => cmdSwitch(true),
  off: () => cmdSwitch(false),
  help: () => out('commands: setup, status, once [--read], daemon, dm, remember, recall, install-skill, on, off'),
};
