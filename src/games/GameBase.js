  ...
  bindKeys() {
    ...
    if (!gamepadBound) {
      ...
      const bindBtn = (id, k) => {
        ...
        if (!btn) return;
        btn.addEventListener('pointerdown', (e) => { e.preventDefault(); this.keys[k] = true; });
        btn.addEventListener('pointerup', (e) => { e.preventDefault(); this.keys[k] = false; });
        btn.addEventListener('pointerleave', () => { this.keys[k] = false; });
      };
